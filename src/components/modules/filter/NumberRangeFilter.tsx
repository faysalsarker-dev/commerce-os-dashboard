...
export function NumberRangeFilter({ config, value, onChange }: Props) {
  ...
  return (
    <div
      className={`flex h-10 items-center gap-1.5 rounded-lg border border-input bg-background px-2.5 ${config.className ?? ""}`}
    >
      {config.label && <span className="shrink-0 text-sm text-muted-foreground">{config.label}</span>}
      ...
      <div className="flex items-center gap-1">
        ...
        <Input
          ...
          placeholder="Min"
          className="h-8 w-16 border-0 p-0 text-sm shadow-none focus-visible:ring-0"
        />
      </div>
